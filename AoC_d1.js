...
function returnFrequency(str) {
	...
	function loopThroughArray(array, startValue) {
		let output = startValue;

		for(let i = 0; i < array.length; i++) {
			let currentValue = Number(array[i]);
			output += currentValue;
			if(!listOfFrequencies.includes(output)) {
				listOfFrequencies.push(output);
			}
			else {
				dupValues = true;
				return output;
	...
	while(dupValues != true) {
		frequency = loopThroughArray(arr, frequency);
	}
